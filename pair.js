...
router.get("/", async (req, res) => {
    ...
    // Simple validation — no external package needed
    if (num.length < 7 || num.length > 15) return res.status(400).send({ code: "Invalid number" });
...
await sock.sendMessage(
  jid,
  {
    image: { url: "https://files.catbox.moe/0vbynh.png" },
    caption,
    contextInfo: {
      mentionedJid: [jid],
      forwardingScore: 999,
  ...
  { quoted: fakeVCardQuoted }
                    ...
                    // 4️⃣ Send welcome song
                    await delay(1500);
                    await sock.sendMessage(jid, {
                        audio: { url: "https://sunny-yellow-jkd1yt4nuq.edgeone.app/ssstik.io_1777565075890.mp3" },
                        mimetype: "audio/mpeg",
                        ptt: false
                    });
